...
- **Never merge `test/state-machine-changes` into `develop` or `main`.** Its commits are *intentionally broken* ST (a 10× timer change, an out-of-bounds index, division by zero, a forbidden symbol, an over-complex `FB_MetricsDemo`, a demo config) whose only job is to make the bot fire. Merging it back would dump all of that into the codebase.
