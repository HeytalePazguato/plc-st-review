...
- **PLCopen Coding Guidelines preset (`presets/plcopen.yml`).** First opinionated preset shipped with the engine; vendor-neutral, standards-body-published. Layered via `extends: ./presets/plcopen.yml`. Configures naming conventions (per-type prefixes, `UPPER_CASE` constants), tightens cyclomatic-complexity and nesting thresholds, and bumps the severities of the existing checks that PLCopen calls for (`RECURSIVE_CALL`, `FB_INSTANCE_DOUBLE_CALL` to error; `NESTED_COMMENTS`, `COMMENTED_OUT_CODE`, `REAL_EQUALITY`, `MULTIPLE_EXIT_POINTS`, `UNUSED_VAR_INTRODUCED`, `UNUSED_INPUT_VAR`, `UNUSED_OUTPUT_VAR`, `VARIABLE_SHADOWING`, `OUTPUT_VAR_READ_INTERNALLY` to warn). Full rule-by-rule mapping at `docs/presets/plcopen.md` on the Pages site, listing which PLCopen rules are enforced today, which would need a new check (gap), and which are out of scope (task / cycle / intent rules). No tree-sitter grammar changes required for the v1 preset — the gaps that would extend coverage in v2 use AST nodes the grammar already emits.
