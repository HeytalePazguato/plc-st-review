...
- **`examples/demo/plcopen_violations.st` + `examples/demo/plcopen-lint.yml`** — a self-contained "do it yourself" sample (one file, five POUs) that deliberately trips every single-revision PLCopen check, so `plc-st-review --lint examples/demo/plcopen_violations.st --config examples/demo/plcopen-lint.yml` prints the full PLCopen check set firing at once.
...
- **`commander` bumped 14.0.3 → 15.0.0** (Dependabot `npm-prod` group, PR #46). Major bump; the CLI's option/argument wiring is unaffected — `--help`, flag parsing, and a full lint run verified against the new major, and all 274 tests pass.
...
- **Parameters were double-counted as locals.** The symbol-table collector walked every `var_section` block (including `VAR_INPUT` / `VAR_OUTPUT` / `VAR_IN_OUT`) and pushed every variable declaration into `pouLocals`, so each parameter appeared as both a `var_input` (via `Pou.inputs`) AND a `var_local` (via `pouLocals`) `NamedDecl`. Two cascades surfaced: `NAME_REUSED_DIFFERENT_KIND` fired on every parameter (`Name 'xCount' is reused across kinds (var_input, var_local)`), and `UNINITIALIZED_VAR_USED` fired on every parameter read (`'xCount' is read before any assignment`) — together ~100 false positives on a typical-shape FB. The collector's local push is now gated to local-storage sections (`VAR_BLOCK`, `VAR_TEMP`, `VAR_EXTERNAL`); timer / counter / edge-trigger / bistable / pointer / array indexes still walk every section so an FB-instance declared as a parameter (uncommon but valid) doesn't go unindexed. Real-parser regression test added.

- **`UNQUALIFIED_ENUM_CONSTANT` was firing on enum value declarations themselves.** The check's "skip refs at `__global` scope" filter didn't catch enum-value identifiers inside `TYPE E_Color : (RED, GREEN, BLUE); END_TYPE` — those got `scope === '<file>'` from `pouContainingLine` (no POU contains the TYPE block) and slipped through, producing "consider writing RED as E_Color.RED" on the very line where E_Color.RED is *defined*. Fix has two parts: (1) `collectVarReferences` now skips identifiers whose parent is an `enumerator` node, so enum value decl-sites don't surface as references at all (they're declarations, not uses); (2) the check's scope filter extends to skip `<file>` scope alongside `__global`. Real-parser regression test added.

- **`COMMENTED_OUT_CODE` was firing on prose comments mentioning ST keywords.** The heuristic matched bare keywords (`IF`, `FOR`, `CASE`, `WHILE`, `VAR`, `PROGRAM`), so any English documentation that said "in a CASE statement", "If you forget to handle this", or "the FOR loop iterates over" was wrongly flagged as commented-out code. The tightened pattern requires real ST shape: `:=` / `=>` operators, `END_X` block terminators (which never appear in prose), `RETURN;` / `EXIT;` / `CONTINUE;` / `GOTO;` (control-transfer with terminator), or full control-flow forms (`IF ... THEN`, `FOR ... :=`, `WHILE ... DO`, `CASE ... OF`). Bare keyword mentions in prose no longer fire. Real-parser regression test added.

- **`FOR_LOOP_VAR_USED_AFTER` used a fixed 200-line "body window" instead of the loop's actual end.** A `iLoop := ...` reference more than 200 lines below the `FOR` header — but still inside the loop body, before `END_FOR` — was wrongly flagged as a post-loop use. The collector now records each loop's actual `END_FOR` line (via the AST node's `endPosition`) on `ForLoop.endLine`; the check uses `ref.line <= loop.endLine` to identify in-body refs precisely. No more false positives on long-bodied loops. Real-parser regression test added with a synthetic 250-line FOR body.

- **`TOO_MANY_GLOBALS_USED` ignored `case_sensitive` when deduplicating.** The check unconditionally lowercased the dedup key when counting distinct globals in a POU, so in case-sensitive dialects (B&R Automation Studio) `myGlobal` and `MyGlobal` (two genuinely distinct globals) collapsed to one. The dedup key now honours `t.caseSensitive`: lowercased in case-insensitive dialects, preserved in case-sensitive ones. Behavior unchanged on the default (case-insensitive) configuration.
